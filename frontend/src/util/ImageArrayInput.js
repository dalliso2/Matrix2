...
export default function ImageArrayInput({field, caseId})
{        
    // field.value will be an array 
    const [imageIds, setImageIds] = useState(field.value);
    ...
    let error = false;
    if (field.error) // may be an array
    {
        if (Array.isArray(field.error))
            error = field.error.reduce((acc, curr) => acc || curr, false);
    }
    ...
    return (
        ...
        <Box className={field.error?"image-array-label-file-upload-error":"image-array-label-file-upload "}>
